...
# Parámetros
# ========================================
file_in = "voluntario2/datos.txt" # Nombre del fichero de datos
file_out = "voluntario2/videos/test" # Nombre del fichero de salida (sin extensión)
...
for planet_pos, radius in zip(frames_data[0], planet_radius):
    ...
    if show_trail:
        planet_trail, = ax.plot(
                ...
                color=planet_points[-1].get_facecolor(), alpha=0.5)
...
def update(j_frame, frames_data, planet_points, planet_trails, show_trail):
    ...
    line.set_data([0, frames_data[j_frame][0][0], frames_data[j_frame][1][0]], [0, frames_data[j_frame][0][1], frames_data[j_frame][1][1]])

    return planet_points + planet_trails + [line]
